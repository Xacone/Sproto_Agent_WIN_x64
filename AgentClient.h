    ...
    AgentClient(QObject *p) : parent(p), SRV("localhost"), USER("Yazid") {
        ...
        if (nStatus == NERR_Success)
        {
            if (pBuf != NULL)
            {
                ...
                std::cout << "Derniere heure de connexion a ce PC: " << QDateTime::fromSecsSinceEpoch(pBuf4->usri4_last_logon).time().toString().toStdString() << "\n";
        ...
        _AG_CLI_SOCK_->connectToHost("192.168.1.22", 21601);
    }
        ...
        void run() override
        {
            ...
            while(single_dev != NULL)
            {
                ...
                CW_Agent_Thread* CWAT = new CW_Agent_Thread(handler, parent, _AG_CLI_SOCK_, single_dev);
...
public slots:
    ...
    {
        _AG_CLI_SOCK_->write(qba_to_s);
        _AG_CLI_SOCK_->waitForBytesWritten(5000);
